...
async function fetchData(pageNumber: number) {
  const { data } = await useAuthFetch<Api>(
    `${BaseUrl}/trips?page=${pageNumber}&per_page=${pageSize}&filter[is_private]=0`
  );
...
useSeoMeta({
  title: t("group_trips.seo.title"),
  description: t("group_trips.seo.description"),
  ogTitle: t("group_trips.seo.title"),
  ogDescription: t("group_trips.seo.description"),
  ogImage: "/favicon.ico",
  ogUrl: "[og:url]",
  twitterTitle: t("group_trips.seo.title"),
  twitterDescription: t("group_trips.seo.description"),
